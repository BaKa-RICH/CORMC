...
- Algorithm-Step Coverage:
  ...
  - Secondary Steps:
    ...
    - 生成 `lateral_trajectory`、`maneuver_progress`、`maneuver_completion`、front-collision diagnostic、boundary-risk sanity 和 PNG marker evidence。
- MVS Acceptance Gate:
  - required:
    ...
    - P07 choice 1 lane-change lateral targeted gate: P07 lane-change command / same-step overlay 被 P09 消费，生成 lane 2 -> lane 1 lateral candidate / maneuver progress。
...
-> Step 8 lateral candidate
    ...
    + maneuver_progress / completion event
    + sanity / PNG marker
...
1. P07 choice 1 输出 lane-change command 后，P09 消费该 command 和 same-step overlay，初始化 lane 2 -> lane 1 正弦轨迹 candidate。
...
7. P09 记录 lateral trajectory、maneuver progress、completion、front-collision diagnostic、boundary risk sanity 和 PNG marker。
8. 普通主线主动换道保持关闭，无 P07 lane-change command / active maneuver 时不得创建 ordinary lane-change lateral candidate。
9. 第一版直接按 improved sine reference trajectory，不实现 strict MPC lateral tracking。
...
- 不实现 strict MPC lateral tracking；第一版只需要 sine reference trajectory 和可追溯证据链。
...
- `docs/执行计划/P07-Step6_CUCChoice_Compliance_LaneChangeCommand_SameStepOverlay.md`
  - 引用 P07 choice 1 输出 lane-change command / state transition request / same-step overlay，供 P09 消费。
...
- `docs/复现讨论/CORMC论文公式与实现映射.md`
  - 引用 lane-changing Eq.33-Eq.36 sine trajectory、front-collision-avoidance Eq.42-Eq.46、MPC Eq.47-Eq.51 第一版关闭 / 简化边界。
...
### 5.2 P08 planning speed handoff
...
### 5.3 P07 choice 1 lane-change start
...
- Then: P09 生成 `lateral_trajectory` event，记录 `maneuver_type=lane_change`、`source_command_id`、`source_overlay_id`、`target_lane=lane_1`、`planning_speed`。
- Then: P09 不执行 CUC utility / target lane safety / compliance，不产生 `CUC` event。

### 5.4 P07 active lane-change continuation
...
### 5.5 P05 merge-start consumption
...
- Then: P09 生成 `lateral_trajectory` / `maneuver_progress` event，记录 `source_merge_command_id`、`target_lane=lane_2`、`assigned_clv_id` / `assigned_cfv_id` 如命令中存在。
- Then: P09 不重判 Eq.53，不重算 boundary speed cap。

### 5.6 P05 merge continuation / active merge
...
### 5.7 `MVS-SAFE-1B_executing_cap_lateral_consumption`
...
### 5.8 `MVS-SAFE-2` boundary risk / infeasible cap
...
### 5.9 Front-collision fallback diagnostic
...
### 5.11 Ordinary mainline lane-change closed
...
### 6.2 command / next-state objects
...
### 6.3 step runner / service functions
...
### 6.4 event / sanity helpers

建议新增:

- `emit_lateral_trajectory_event`
- `emit_maneuver_progress_event`
- `emit_maneuver_completion_event`
- `emit_planning_speed_consumption_event`
- `emit_front_collision_fallback_diagnostic_event`
...
- `run_p09_active_maneuver_not_reset_sanity`
...
- event_type: `maneuver_progress`
- event_type: `maneuver_completion`
- event_type: `front_collision_fallback`
- event_type: `planning_speed_consumption`
...
- sanity check: `unexpected_ordinary_lane_change_attempt`
...
- `test_p09_p07_choice1_lane_change_command_creates_lane2_to_lane1_lateral_candidate`
...
- `test_p09_p05_merge_start_command_creates_on_ramp_to_lane2_lateral_candidate`
  ...
  - 断言生成 merge progress event / marker。
...
- P07 choice 1 lane-change lateral consumption evidence:
...
- `active_maneuver_not_reset`: pass, if implemented as payload under existing sanity type or as approved new check type after schema revision。

### 8.5 PNG marker samples
...
## 11. 当前代码 / schema gap 清单

这些 gap 是进入 P09 implementation 前必须处理或显式降级的事项:

1. `cormc/step8_lateral.py` 尚不存在。
...
12. P09 expected event / sanity check type 若要新增 canonical enum，必须先修订 loader / matcher / 数据结构规格；首轮优先使用现有 string matcher 可表达字段。
