...
from cormc.simulation_core.aps import run_step4a_aps_for_scenario


def test_mvs_aps_fail_empty_p04_required_fails_until_aps_implemented() -> None:
    report = _run_p04_report(_aps_fail_empty_config())
...
def test_mvs_aps_fail_cache_p04_required_fails_until_aps_implemented() -> None:
    report = _run_p04_report(_aps_fail_cache_config())
...
def test_p04_merge_zone_handoff_refreshes_bounded_assignment_before_cmc() -> None:
    result = run_step4a_aps_for_scenario(_merge_zone_handoff_refresh_config())

    trigger_event = _actual_event(
        result.actual_events,
        "APS",
        "MV_HANDOFF_REFRESH",
        reason="merge_zone_handoff",
    )
    assignment = result.assignment_views["MV_HANDOFF_REFRESH"].record

    assert trigger_event["payload"]["trigger"] == "merge_zone_handoff"
    assert assignment["source"] == "aps_updated_this_step"
    assert assignment["lifecycle_state"] == "active_merge_zone"
    assert assignment["created_at_step"] == 1
    assert assignment["clv_id"] == "CLV_HANDOFF_REFRESH"
    assert assignment["cfv_id"] == "CFV_HANDOFF_REFRESH"
    assert result.assignment_views["MV_HANDOFF_REFRESH"].consumable_by_cmc is True
    assert result.cache_actions[0].action == "update_request"
...
def test_first_aps_event_marked_engineering_patch() -> None:
    report = _run_p04_report(_first_aps_trigger_config())
...
def _run_p04_report(config: dict[str, Any]):
...
def _merge_zone_handoff_refresh_config() -> dict[str, Any]:
    return _base_config(
        "P04-MERGE-ZONE-HANDOFF-REFRESH",
        vehicles=[
            _vehicle(
                "MV_HANDOFF_REFRESH",
                "on_ramp",
            ...
            _vehicle("CLV_HANDOFF_REFRESH", "lane_2", 7010.0, 0.0),
            _vehicle("CFV_HANDOFF_REFRESH", "lane_2", 6930.0, 0.0),
        ],
        preloaded_assignments=[
            {
                "mv_id": "MV_HANDOFF_REFRESH",
                "clv_id": "CLV_HANDOFF_REFRESH",
                "cfv_id": "CFV_HANDOFF_REFRESH",
                "aps_case": "case_3",
                "col_clv": True,
                "col_cfv": False,
                "desired_spacing_override": None,
        ...
        expected_events=[],
        expected_sanity_checks=[],
    ) | {"initial_time": {"t": 0.1, "step": 1, "dt": 0.1}}
