...
from cormc.p11_output import run_full_required_mvs_smoke_suite
from cormc.random_generation import P16_DEMO_SCENARIO_ID
...
def test_p16_random_disabled_required_mvs_suite_remains_green() -> None:
    suite = run_full_required_mvs_smoke_suite()
    assert len([item for item in suite.scenario_results if item.passed]) == 20
