...
from cormc.simulation_core.longitudinal import run_step7_longitudinal_model_spacing_speedcap
...
def test_p08_mv_assignment_safety_cap_prevents_passing_clv_in_control_zone() -> None:
    state, relations = _state_and_relations(
        _p08_config(
            mv_x=6848.0,
            mv_initial_v=30.0,
            ...
            cfv_x=6810.0,
            preloaded_assignments=[
                _aps_assignment("MV_CUC", aps_case="case_3", d_star_clv=-20.0)
            ],
        )
    )
    relations = overlay_assignment_logical_relations(state, relations)
    ...
    event = _event(result.actual_events, event_type="longitudinal_model", vehicle_id="MV_CUC")
    payload = event["payload"]
    assert "assignment_physical_safety_cap" in candidate.constraints_applied
    assert payload["assignment_physical_safety_cap_applied"] is True
    assert payload["assignment_physical_safety_leader_id"] == "CLV_Y"
    assert payload["planning_speed"] == candidate.planning_speed
    assert candidate.x_global < state.vehicle_states["CLV_Y"].x_global


def test_p08_control_zone_cfv_keeps_physical_leader_until_merge_zone() -> None:
    state, relations = _state_and_relations(
        _p08_config(
            mv_x=6840.0,
            mv_initial_v=18.0,
    ...
    payload = event["payload"]
    assert payload["leader_id"] == "CLV_Y"
    assert payload["leader_relation_source"] == "lane_ordering"
    assert payload["desired_spacing_target_source"] == "Eq18"
    assert payload["spacing_override_consumed"] is False
    assert payload["longitudinal_mode"] == "cav_gap_regulating"
    assert payload["actual_spacing_d_i"] > 2.0
