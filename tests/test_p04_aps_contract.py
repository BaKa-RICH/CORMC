...
def test_mvs_aps_fail_empty_p04_required_fails_until_aps_implemented() -> None:
    report = run_targeted_scenario(
        _aps_fail_empty_config(),
        actual_events=[],
        actual_sanity_checks=[],
    )

    _assert_matcher_layer_required_failure(report, expected_event="APS")


def test_mvs_aps_fail_cache_p04_required_fails_until_aps_implemented() -> None:
    report = run_targeted_scenario(
        _aps_fail_cache_config(),
        actual_events=[],
        actual_sanity_checks=[],
    )

    _assert_matcher_layer_required_failure(report, expected_event="APS")
    assert any("assignment_cache_overwrite_by_failed_APS" in reason for reason in report.failure_reasons)
...
def test_mvs_aps_case_2_eq10_to_cfv_only_contract() -> None:
    ...
    _assert_matcher_layer_required_failure(report, expected_event="APS")
    eq10_expected = _expected_event(report, "APS", {"aps_case": "case_2"})
    assert eq10_expected["match"]["eq10_vehicle_role"] == "cfv"
...
def test_mvs_aps_case_3_no_eq10_to_clv_contract() -> None:
    ...
    _assert_matcher_layer_required_failure(report, expected_event="APS")
    forbidden_result = _matcher_result(report, "forbidden_events")
    assert forbidden_result.passed is True
    assert forbidden_result.issues == []
...
def test_mvs_aps_case_4_eq10_to_cfv_only_contract() -> None:
    ...
    _assert_matcher_layer_required_failure(report, expected_event="APS")
    eq10_expected = _expected_event(report, "APS", {"aps_case": "case_4"})
    assert eq10_expected["numeric_expectations"]["desired_spacing_override"]["value"] == 37.6


def test_first_aps_event_marked_engineering_patch() -> None:
    config = _first_aps_trigger_config()
    report = run_targeted_scenario(config, actual_events=[], actual_sanity_checks=[])

    _assert_matcher_layer_required_failure(report, expected_event="APS")
    trigger_event = _expected_event(report, "APS", {"trigger": "first_APS"})
    assert trigger_event["reason_code"] == "first_aps"
    assert trigger_event["source"] == "first_version_engineering_patch"
...
def test_non_aps_period_reuses_cache_without_mutating_state() -> None:
    ...
    _assert_matcher_layer_required_failure(report, expected_event="APS")
    reuse_event = _expected_event(report, "APS", {"trigger": "reuse_cache"})
    assert reuse_event["match"]["effective_assignment_source"] == "cache_reused"
...
def test_p04_does_not_write_vehicle_state_before_commit() -> None:
    ...
    assert _state_signature(frozen) == before_signature
    _assert_matcher_layer_required_failure(report, expected_event="APS")
    assert any("p04_no_write_before_commit" in reason for reason in report.failure_reasons)


def test_p04_hands_off_merging_zone_or_executing_mv_to_cmc_contract() -> None:
    ...
    assert _state_signature(frozen) == before_signature
    _assert_matcher_layer_required_failure(report, expected_event="APS")
    handoff_event = _expected_event(report, "APS", {"branch": "handed_off_to_cmc"})
    assert handoff_event["reason_code"] == "mv_in_merging_zone"
    assert handoff_event["source"] == "first_version_engineering_patch"


def _assert_matcher_layer_required_failure(report: Any, *, expected_event: str) -> None:
    assert report.status == "required"
    assert report.classification == "required_failed"
    assert report.blocks_required_suite is True
    assert any(f"missing expected event: {expected_event}" in reason for reason in report.failure_reasons)
    issue_codes = [
        issue.code
        for matcher_result in report.matcher_results
        for issue in matcher_result.issues
    ]
    assert "missing_event" in issue_codes
    assert set(issue_codes).issubset({"missing_event", "missing_sanity_check"})
...
def _base_config(
    ...
    *,
    ...
    return {
        ...
        "purpose": "P04 APS contract red test",
        "test_level": "unit",
        "status": "required",
        "derivation_ref": ["CORMC最小验证场景执行规格.md#5"],
...
def _aps_fail_cache_config() -> dict[str, Any]:
    return _base_config(
        "MVS-APS-FAIL-CACHE",
        vehicles=[
            _vehicle("MV_FAIL_CACHE", "on_ramp", 6830.0, -3.5, road_role="on_ramp_mv", merge_state="not_started"),
...
def _aps_fail_empty_config() -> dict[str, Any]:
    return _base_config(
        "P04-CONTRACT-APS-FAIL-EMPTY",
        vehicles=[
...
def _aps_case_config(
    *,
    ...
    return _base_config(
        scenario_id,
        vehicles=[
            _vehicle("MV_A", "on_ramp", 6850.0, -3.5, road_role="on_ramp_mv", merge_state="not_started"),
...
def _first_aps_trigger_config() -> dict[str, Any]:
    return _base_config(
        ...
        vehicles=[
            _vehicle("MV_FIRST", "on_ramp", 6850.0, -3.5, road_role="on_ramp_mv", merge_state="not_started"),
...
def _reuse_cache_config() -> dict[str, Any]:
    return _base_config(
        "P04-REUSE-CACHE",
        vehicles=[
            _vehicle("MV_CACHE", "on_ramp", 6890.0, -3.5, road_role="on_ramp_mv", merge_state="not_started"),
        ...
        preloaded_state_machine_states=[
            {"vehicle_id": "MV_CACHE", "last_aps_time": 0.0, "notes": "t=2.0 would reuse cache"}
        ],
        expected_events=[
            {
                "event_type": "APS",
                ...
                "match": {"trigger": "reuse_cache", "effective_assignment_source": "cache_reused"},
                "source": "first_version_engineering_patch",
            }
        ],
        expected_sanity_checks=[
...
def _p04_no_write_contract_config() -> dict[str, Any]:
    return _base_config(
        ...
        vehicles=[
            _vehicle("MV_NOWRITE", "on_ramp", 6850.0, -3.5, road_role="on_ramp_mv", merge_state="not_started"),
...
def _expected_event(report: Any, event_type: str, match_subset: dict[str, Any]) -> dict[str, Any]:
    for matcher_result in report.matcher_results:
        for issue in matcher_result.issues:
            ...
            if all(match.get(key) == value for key, value in match_subset.items()):
                return expected
    raise AssertionError(f"missing expected event contract: {event_type} {match_subset}")
