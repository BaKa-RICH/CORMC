...
def test_p10_mvs_e2e_1_helper_chain_commits_merge_start_handoff() -> None:
    ...
    assignment = config["preloaded_assignments"][0]
    helper_route_evidence = {
        "target_gate": "MVS-E2E-1",
        "route": "helper_targeted",
        "runner_route_available": False,
        "runner_loader_gap": "MVS-E2E-1 built-in P08/P09/P10 route not registered",
        "aps_case": assignment["aps_case"],
        "aps_assignment_source": assignment["source"],
    }

    p05 = run_step4b_cmc(state, relations, config=config)
    p08 = run_step7_longitudinal_model_spacing_speedcap(
        state,
        relations,
        command_buffer=p05.command_buffer,
    ...
    p10_buffer = NextStateBuffer(
        step=state.step,
        t=state.t,
        candidate_longitudinal=p08.next_state_buffer.candidate_longitudinal,
        candidate_lateral=p09.next_state_buffer.candidate_lateral,
    ...
    result = commit_step(
        ...
        p05.command_buffer,
        p10_buffer,
        run_id="P10-MVS-E2E-1-HELPER",
        scenario_id="MVS-E2E-1",
    )

    eq53_event = _dict_event(p05.actual_events, event_type="CMC", reason="eq53_gap")
    merge_event = _dict_event(p05.actual_events, event_type="CMC", reason="merge_start")
    p08_candidate = p08.next_state_buffer.candidate_longitudinal["MV_CMC_1"]
    p09_candidate = p09.next_state_buffer.candidate_lateral["MV_CMC_1"]
    p09_progress = p09.next_state_buffer.candidate_maneuver_progress["MV_CMC_1"]
    final_candidate = result.final_candidates["MV_CMC_1"]
    committed = result.next_state.vehicle_states["MV_CMC_1"]
    active = result.next_state.active_maneuvers["MV_CMC_1"]
    commit_event = _event(result.history.event_records, event_type="commit", vehicle_id="MV_CMC_1")

    assert helper_route_evidence["route"] == "helper_targeted"
    assert helper_route_evidence["runner_route_available"] is False
    assert helper_route_evidence["aps_case"] == "case_1"
    assert helper_route_evidence["aps_assignment_source"] == "test_preload"
    assert eq53_event["payload"]["eq53_pass"] is True
    assert merge_event["payload"]["command_id"] == "p05:0:merge_start:MV_CMC_1"
    assert merge_event["payload"]["state_transition_command_id"] == "p05:0:merge_state:MV_CMC_1"
    assert p08_candidate.candidate_id == "p08:0:MV_CMC_1:longitudinal"
    assert p08_candidate.source == "step7_longitudinal_model"
    assert "p05:0:speed_cap:MV_CMC_1" in p08_candidate.source_commands
    assert p09_candidate.candidate_id == "p09:0:MV_CMC_1:lateral"
    assert p09_candidate.source == "step8_lateral_trajectory"
    assert "p05:0:merge_start:MV_CMC_1" in p09_candidate.source_commands
    assert p08_candidate.candidate_id in p09_candidate.source_commands
    assert p09_progress.source_command_id == "p05:0:merge_start:MV_CMC_1"
    assert final_candidate.candidate_id == "p10:0:MV_CMC_1:final"
    assert final_candidate.source_longitudinal_candidate == p08_candidate.candidate_id
    assert final_candidate.source_lateral_candidate == p09_candidate.candidate_id
    assert committed.x_global == p08_candidate.x_global
    assert committed.y == p09_candidate.y
    assert committed.merge_state == "executing"
    assert active.source_command_id == "p05:0:merge_start:MV_CMC_1"
    assert active.progress == p09_progress.progress
    assert active.last_planning_speed == p08_candidate.planning_speed
    assert commit_event.payload["source_longitudinal_candidate"] == p08_candidate.candidate_id
    assert commit_event.payload["source_lateral_candidate"] == p09_candidate.candidate_id
    assert commit_event.payload["source_state_transition_command"] == "p05:0:merge_state:MV_CMC_1"
    assert commit_event.payload["active_maneuver_persisted_vehicle_ids"] == ["MV_CMC_1"]
    assert result.history.trajectory_records[0].vehicle_id == "MV_CMC_1"
    ...
    assert {"commit_marker", "trajectory_quicklook", "active_maneuver_marker", "source_chain_marker"}.issubset(
        {feature["feature_type"] for feature in result.expected_png_features}
    )
    assert not _dict_has_event_type(
        p05.actual_events + p08.actual_events + p09.actual_events + result.history.event_dicts(),
        "CUC",
    )
    assert _forbidden_events_absent(
        result,
        {"APS", "CMC", "CUC", "longitudinal_model", "speed_cap", "lateral_trajectory"},
    )
