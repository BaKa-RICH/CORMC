...
from cormc.traffic_flow.generation import ArrivalStream, SeededRandomProfile, compute_spawn_decisions, generate_boundary_queue
...
def test_spawn_real_sumo_adds_generated_once_and_duplicate_does_not_add(tmp_path: Path) -> None:
    ...
    files = build_sumo_network(tmp_path, SumoNetworkConfig(end=1.0))
    traci = import_traci()
