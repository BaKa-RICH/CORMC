...
from cormc.engine import CormcEngine, build_initial_state_from_scenario_config
...
def test_p15_full_minimal_null_recorders_do_not_change_engine_result() -> None:
    scenario = load_builtin_scenario("MVS-CUC-2")
    ...
    for recorder in (full, minimal, null):
        recorder.record_step(step_result, run_id="p15-recording", scenario_id="MVS-CUC-2")
...
def test_p15_engine_hot_path_has_no_file_output_calls() -> None:
    source = Path("cormc/engine.py").read_text(encoding="utf-8")
    tree = ast.parse(source)
    forbidden_import_modules = {"cormc.p11_output", "cormc.p14_artifacts"}
    imported_modules = {
        node.module
