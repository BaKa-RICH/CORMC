...
from cormc.legacy.basic_numeric import run_basic_numeric_scenario, run_basic_numeric_suite
...
def test_basic_04_numeric_summary_extracts_main_chain(tmp_path: Path) -> None:
    ...
    first_assignment = summary["assignment_validity_timeline"][0]["payload"]
    assert first_assignment["assignment_valid"] is True
    assert first_assignment["assigned_cfv_id"] == "B04_CFV"
    assert first_assignment["invalid_reason"] is None
    assert summary["aps_excluded_candidate_timeline"] == []
    assert summary["first_cached_boundary_invalidation"] is None
    assert summary["cuc_choice_timeline"]
    assert {
        item["final_choice"] for item in summary["cuc_choice_timeline"] if item["vehicle_id"] == "B04_CFV"
    } == {"stay_lane_2"}
...
def test_basic_05_merge_zone_refresh_chain_merges_with_local_cfv(tmp_path: Path) -> None:
    summary = run_basic_numeric_scenario(
        "BASIC-05",
        output_dir=tmp_path,
        run_id="basic-test",
        max_steps=900,
    ...
    assert summary["observed_aps_case"] == "case_3"
    assert summary["merge_zone_refresh_chain_tolerated"] is True
    assert summary["eq10_consumers"] == ["B05_CFV"]
    assert summary["illegal_eq10_consumers"] == []
    assert summary["bounded_assignment_merge_success"] is True
    assert summary["merge_success_gap_type"] == "bounded"
    assert summary["merge_success_clv_id"] == "B05_CLV"
    assert summary["merge_success_cfv_id"] == "B05_CFV"
    assert summary["used_front_only_recovery_for_success"] is False
    assert any(
        item["aps_case"] == "case_2"
        and item["clv_id"] == "B05_CLV"
        and item["cfv_id"] == "B05_CFV"
        for item in summary["aps_assignment_timeline"]
    )
