...
from cormc.simulation_core.pre_freeze import DEFAULT_ROAD_GEOMETRY
...
def test_p16_profile_mapping_round_trip_preserves_stream_extensions() -> None:
    profile = profile_from_mapping(
        {
            "enabled": True,
            "seed": 42,
            "profile_id": "unit-extended",
            "arrival_streams": [
                {
                    "lane_id": "on_ramp",
                    "shifted_headway": 3.5,
                    ...
                    "spawn_y": -3.5,
                    "mean_headway": 6.0,
                    "vehicle_id_prefix": "unit_prefix",
                    "road_role": "on_ramp_mv",
                    "merge_state": "not_started",
                    "vehicle_id_lane_label": "ramp",
                }
            ],
        }
    )

    round_trip = profile_from_mapping(profile_to_dict(profile))
    queue = generate_boundary_queue(round_trip, max_t=20.0)

    assert round_trip.arrival_streams[0].vehicle_id_prefix == "unit_prefix"
    assert round_trip.arrival_streams[0].road_role == "on_ramp_mv"
    assert round_trip.arrival_streams[0].merge_state == "not_started"
    assert round_trip.arrival_streams[0].vehicle_id_lane_label == "ramp"
    assert queue
    assert queue[0].vehicle_id.startswith("unit_prefix_ramp_")
    assert queue[0].initial_state.road_role == "on_ramp_mv"
