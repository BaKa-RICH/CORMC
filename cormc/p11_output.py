...
from cormc.mvs import ScenarioConfigError, load_builtin_scenario, run_targeted_scenario
from cormc.mvs.runner import ScenarioReport
from cormc.step0_3 import DEFAULT_ROAD_GEOMETRY, RoadGeometryConfig
...
P11_PROBE_MVS_IDS: tuple[str, ...] = ("MVS-CUC-1B_real_utility_probe",)
P11_DEFERRED_MVS_IDS: tuple[str, ...] = ("MVS-CUC-1C_real_utility_choice1_locked",)
P11_EXTRA_DIAGNOSTIC_IDS: tuple[str, ...] = ("P05-EXECUTING-CONTINUATION",)
...
def _run_required_scenario(scenario_id: str) -> ScenarioAggregationResult:
    try:
        ...
        return _blocked_required_scenario(
            ...
            detail=_missing_route_detail(scenario_id),
        )

    loader_status = str(config["status"])
    if loader_status != "required":
        return _blocked_required_scenario(
            scenario_id,
            reason="classification_mismatch",
            detail=(
                f"upstream P11 denominator marks {scenario_id} required, "
                f"but current loader status {loader_status}"
            ),
        )
