...
def build_next_simulation_state(
    ...
    *,
    ...
    return SimulationState(
        ...
        ramp_merge_runtime=(
            next_state_buffer.ramp_merge_runtime
            if next_state_buffer.ramp_merge_runtime is not None
            else state.ramp_merge_runtime
        ),
    )
...
def _state_signature(state: SimulationState) -> tuple[Any, ...]:
    return (
        ...
        _runtime_signature_payload(state.ramp_merge_runtime),
    )
...
def _runtime_signature_payload(value: Any) -> Any:
    if is_dataclass(value):
        return {
            field.name: _runtime_signature_payload(getattr(value, field.name))
            for field in fields(value)
        }
    if isinstance(value, MappingABC):
        return tuple(
            (key, _runtime_signature_payload(item))
            for key, item in sorted(value.items(), key=lambda entry: repr(entry[0]))
        )
    if isinstance(value, (list, tuple)):
        return tuple(_runtime_signature_payload(item) for item in value)
    try:
        hash(value)
    except TypeError:
        return repr(value)
