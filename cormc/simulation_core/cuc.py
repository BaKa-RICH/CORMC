...
from cormc.simulation_core.longitudinal import SpacingOverrideConsumption, compute_p145_longitudinal_formula
...
def run_step6_cuc_choice_compliance_lane_change_overlay(
    ...
    *,
    ...
    for cv_id in sorted(active_requests):
        ...
        preserve_boundary = _must_preserve_merge_boundary(request)
        if not accepted_by_vehicle:
            effective_choice = "not_applicable"
            fallback_reason = "non_compliant_chv"
        elif preserve_boundary and recommended_choice == "change_to_lane_1":
            effective_choice = "stay_lane_2"
            fallback_reason = "merge_zone_boundary_protection"
        ...
        if fallback_reason == "merge_zone_boundary_protection":
            sanity_checks.append(
                _sanity(
                    state,
                    "state_machine_inconsistency",
                    "pass",
                    vehicle_ids=(cv_id,),
                    reason="merge_zone_boundary_protection",
                    payload={
                        "vehicle_id": cv_id,
                        "source_mv_id": request.get("source_mv_id"),
                        "cv_role": request.get("cv_role"),
                        "lane_change_command_created": False,
                        "spacing_override_command_created": (
                            request.get("desired_spacing_override") is not None
                        ),
                    },
                )
            )
...
def _must_preserve_merge_boundary(request: Mapping[str, Any]) -> bool:
    if request.get("mv_in_merging_zone") is not True:
        return False
    cv_role = str(request.get("cv_role") or "").lower()
    if cv_role not in {"clv", "cfv"}:
        return False
    return str(request.get("aps_case") or "").lower() in {
        "case_2",
        "case_3",
        "case_4",
    }
...
def _decision_reason(decision: Mapping[str, Any]) -> str:
    ...
    if decision.get("fallback_reason") == "merge_zone_boundary_protection":
        return "fallback_merge_zone_boundary_protection"
    if decision.get("fallback_reason") == "non_compliant_chv":
