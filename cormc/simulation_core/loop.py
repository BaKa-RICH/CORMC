...
from cormc.traffic_flow.generation import (
    ...
    DEFAULT_P16_SEED,
    DEFAULT_RANDOM_6450_SEED,
    P16_DEMO_SCENARIO_ID,
    RANDOM_6450_SCENARIO_ID,
    SeededRandomProfile,
    build_p16_demo_scenario_config,
    build_random_6450_scenario_config,
    generate_boundary_queue,
    profile_from_mapping,
)
from cormc.simulation_core.recording import FullRecorder
...
class SimulationLoopConfig:
    ...
    precomputed_random_queue: tuple[BoundaryQueueItem, ...] | None = None
...
def run_deterministic_simulation(config: SimulationLoopConfig) -> SimulationLoopResult:
    ...
    if config.render_png:
        from cormc.legacy.artifact_reports import render_time_space_png
...
def run_seeded_random_simulation(config: SimulationLoopConfig | None = None) -> SimulationLoopResult:
    ...
    if config.precomputed_random_queue is not None:
        random_queue = config.precomputed_random_queue
    else:
...
def _load_loop_scenario_config(config: SimulationLoopConfig) -> dict[str, Any]:
    ...
    if isinstance(scenario, str) and scenario == RANDOM_6450_SCENARIO_ID:
        return load_scenario_config(
            build_random_6450_scenario_config(seed=config.seed or DEFAULT_RANDOM_6450_SEED)
        )
    if isinstance(scenario, str):
        return load_scene_config(scenario)
