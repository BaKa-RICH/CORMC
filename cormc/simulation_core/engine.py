...
from cormc.simulation_core.aps import APSRunResult, run_step4a_aps
from cormc.simulation_core.cmc import Step4BCMCRunResult, run_step4b_cmc
...
from cormc.simulation_core.cuc import Step6CUCRunResult, run_step6_cuc_choice_compliance_lane_change_overlay
from cormc.simulation_core.longitudinal import Step7LongitudinalRunResult, run_step7_longitudinal_model_spacing_speedcap
from cormc.simulation_core.lateral import Step8LateralRunResult, run_step8_lateral_trajectory_planning_speed_progress
...
class CormcEngine:
    ...
    def advance_one_step(self, state: SimulationState) -> EngineStepResult:
        ...
        aps_eligible_mv_ids = tuple(
            ...
            if (
                region.aps_allowed
                or _requires_merge_zone_handoff_aps_refresh(
                    frozen,
                    mv_id,
                    geometry=self.geometry,
                )
...
def _requires_merge_zone_handoff_aps_refresh(
    state: SimulationState,
    mv_id: str,
    *,
    geometry: RoadGeometryConfig = DEFAULT_ROAD_GEOMETRY,
    ...
    if region.region != "merge_zone":
        return False
    if not _is_first_merge_zone_entry_step(state, mv_id, geometry=geometry):
        return False
    record = state.assignment_records_by_mv.get(mv_id)
    if not isinstance(record, Mapping):
        return False
    return (
        str(record.get("gap_type") or "bounded").lower() == "bounded"
        and str(record.get("status") or "valid").lower() in {"valid", "available", "ok"}
        and str(record.get("lifecycle_state") or "active_control_zone").lower()
        in {"active_control_zone", "refresh_failed_retained"}
    )
