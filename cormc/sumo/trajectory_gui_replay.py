...
class TrajectoryGuiReplaySummary:
    status: str
    sumocfg_file: str
    replay_jsonl: str
...
def run_trajectory_gui_replay(
    ...
    *,
    ...
    """Open SUMO-GUI and replay observation trajectory records through TraCI."""
    ...
    return TrajectoryGuiReplaySummary(
        status="ok",
...
def _read_replay_records(path: Path) -> list[dict[str, Any]]:
    ...
    if not records:
        raise ValueError(f"No trajectory replay records found in {path}")
    return records
...
def _sumo_position_for_replay_record(
    ...
    try:
        ...
        if not isinstance(hint, dict) or hint.get("mode") not in {
            "allow_pre_control_on_ramp",
            "allow_open_on_ramp_after_merge_end",
        }:
            raise
        edge_id = str(hint.get("edge_id", "ramp_pre"))
        lane_index = int(hint.get("lane_index", 0))
        lane_position = float(hint.get("lane_position", 0.0))
        return edge_id, lane_index, lane_position
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Replay an observation trajectory artifact in SUMO-GUI.")
    ...
    try:
        ...
        summary = TrajectoryGuiReplaySummary(
            status="failed",
